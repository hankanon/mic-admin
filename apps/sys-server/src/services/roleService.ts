...
import type { RoleCreateInput, RoleUpdateInput } from '../schemas'
...
export function list(): RoleView[] {
  return db.roles.map(toView)
}

export function get(id: number): RoleView {
  const role = findRole(id)
  if (!role) throw new ApiError('角色不存在', 40400)
  return toView(role)
}

export function create(input: RoleCreateInput): Role {
...
export function update(id: number, input: RoleUpdateInput): Role {
  const role = findRole(id)
  if (!role) throw new ApiError('角色不存在', 40400)
